...
all:  clean
	...
	apprepo --destination=$(PWD)/build appdir boilerplate libatk1.0-0 libglib2.0-0 shared-mime-info libffi7 \
	...
	wget --output-document="$(PWD)/build/build.tar.bz2" "https://download.mozilla.org/?product=firefox-esr-latest-ssl&os=linux64"
	...
	export ARCH=x86_64 && $(PWD)/bin/appimagetool.AppImage $(PWD)/build/Boilerplate.AppDir $(PWD)/Firefox-ESR.AppImage
	chmod +x $(PWD)/Firefox-ESR.AppImage
